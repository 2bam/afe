...
supported_ext = ['.zip']
dirs = [item for item in os.listdir(mconf.update_folder) if os.path.isdir(os.path.join(mconf.update_folder, item))]
for game_name in dirs:
    print(_('\nProcesando: {game})').format(game=game_name))
    dir = os.path.join(mconf.update_folder, game_name)
    ...
    if not files:
        ...
        print(_('Ultima version encontrada: {version!s}').format(version=game_list[0]['version']))
